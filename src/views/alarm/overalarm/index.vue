...
const Overalarm = ref<RolePageVO[]>();
// 创建包含固定数据的数组
const fixedOveralarm = [
  {
    id: 1,
    sszh:'测试',
...
Overalarm.value = fixedOveralarm;
...
function exportToExcel(data: any[]) {
  ...
  saveAsExcelFile(excelBuffer, '已处理报警.xlsx');
}
...
function handleDelete(id: number) {
  const index = Overalarm.value.findIndex((item) => item.id === id);
  if (index !== -1) {
    Overalarm.value.splice(index, 1); // 从Overalarm数组中删除对应的数据
  }
}
    ...
    <div class="search-container">
      <el-form ref="queryFormRef" :model="queryParams" :inline="true">
        ...
        <el-form-item>
          ...
          <el-button class="ml-3" @click="exportToExcel(fixedOveralarm);"><template #icon><i-ep-download /></template>导出</el-button>
